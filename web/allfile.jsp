...
<nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> 所有文件
    <span class="c-gray en">&gt;</span> 文件列表
...
<div class="page-container">
    <form action="BaseServlet" method="post" class="text-c"> 日期范围：
        ...
        <input type="hidden" name="action" value="allfile">
    ...
    <div align="center">
        ...
        <c:if test="${page.currentPage>1 }">
            <a href="/BaseServlet?method=page&action=allfile&page=${page.currentPage-1}">上一页</a>
        </c:if>
        <a href="/BaseServlet?method=page&action=allfile&page=${1}">首页</a>
        <c:forEach begin="1" end="${page.totalPage }" step="1" var="i">
            <c:if test="${page.currentPage==i }">
                <a href="/BaseServlet?method=page&action=allfile&page=${i}"><font color="#ff0000">${i}</font></a>
            </c:if>
            <c:if test="${page.currentPage!=i }">
                <a href="/BaseServlet?method=page&action=allfile&page=${i}">${i}</a>
            </c:if>
        </c:forEach>
        <a href="/BaseServlet?method=page&action=allfile&page=${page.totalPage}">末页</a>
        <c:if test="${page.currentPage< page.totalPage }">
            <a href="/BaseServlet?method=page&action=allfile&page=${page.currentPage+1}">下一页</a>
        </c:if>
    </div>
